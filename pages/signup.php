    ...
    if (empty($name) || empty($email) || empty($password)) {
        ...
        if ($stmt->fetch()) {
            ...
            try {
                ...
                $pdo->commit();
                $success = "Registration successful! You can now <a href='" . BASE_URL . "/pages/login.php' style='color: var(--primary); font-weight:bold;'>log in</a>.";
            } catch (Exception $e) {
                $pdo->rollBack();
        ...
        <div class="login-right-panel">
            <div class="login-form-box">
                <div class="login-tab-headers">
                    <a href="<?php echo BASE_URL; ?>/pages/login.php" class="login-tab-link">Log in</a>
                    <a href="signup.php" class="login-tab-link active">Sign up</a>
                </div>
                ...
                <?php if ($success): ?>
                    <div class="alert alert-success" style="background: rgba(16, 185, 129, 0.2); border: 1px solid rgba(16, 185, 129, 0.4); color: #a7f3d0; border-radius: 30px; padding: 15px 20px; font-weight: 600; font-size: 0.95rem; margin-bottom: 20px; text-align: center; line-height: 1.5;">
                ...
                <?php else: ?>
                    <form method="POST" action="" id="signup-form">
                        ...
                        <div class="login-input-group">
                            <select name="role" id="role-select" class="login-input-field" onchange="toggleAgencyFields()">
                            ...
                            <i class="fas fa-user-tag login-input-icon"></i>
                        </div>

                        <!-- Name -->
                        <div class="login-input-group">
                            <input type="text" name="name" required placeholder="Enter full name" class="login-input-field" value="<?php echo isset($_POST['name']) ? htmlspecialchars($_POST['name']) : ''; ?>">
                            <i class="fas fa-user login-input-icon"></i>
                        </div>

                        <!-- Company Name (Agency Only) -->
                        <div class="login-input-group" id="agency-fields" style="display: none;">
                            <input type="text" name="company_name" placeholder="Enter company name" class="login-input-field" value="<?php echo isset($_POST['company_name']) ? htmlspecialchars($_POST['company_name']) : ''; ?>">
                            <i class="fas fa-building login-input-icon"></i>
                        </div>

                        <!-- Email -->
                        <div class="login-input-group">
                            <input type="email" name="email" required placeholder="Enter email address" class="login-input-field" value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email']) : ''; ?>">
                            <i class="fas fa-envelope login-input-icon"></i>
                        </div>
                        
                        <!-- Password -->
                        <div class="login-input-group">
                            <input type="password" name="password" required placeholder="Enter password (min 6 chars)" class="login-input-field" minlength="6">
                            <i class="fas fa-lock login-input-icon"></i>
                        </div>
                        
                        <button type="submit" class="login-green-btn">Create Account</button>
                    </form>
                <?php endif; ?>
            </div>
        </div>
